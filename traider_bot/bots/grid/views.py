...
from api_2.api_aggregator import  get_current_price, min_qty_check
...
from bots.grid.forms import GridForm
...
@login_required
def grid_bot_create(request):
    title = 'Создание Грид бота'
    user = request.user

    if request.method == 'POST':
        bot_form = BotModelForm(request=request, data=request.POST)
        grid_form = GridForm(data=request.POST)

        if bot_form.is_valid() and grid_form.is_valid():
            ...
            if qty_status:
                grid_model = grid_form.save(commit=False)
                grid_model.bot = bot
                grid_model.save()
    ...
    return render(request, 'grid/create.html', {
        'bot_form': bot_form,
        'grid_form': grid_form,
        'title': title,
    })
...
@login_required
def grid_bot_edit(request, bot_id):
