...
from api.api_v5 import get_list, cancel_all, switch_position_mode, set_leverage, get_current_price, set_trading_stop, \
...
class StepHedgeClassLogic:
    ...
    def place_tp_order(self, position_number):
        ...
        if position_idx == 1:
            self.tp_price_dict[position_number] = round(psn_avg_price * Decimal(1 + self.tp_pnl_percent / 1000), self.round_number)
        else:
            self.tp_price_dict[position_number] = round(psn_avg_price * Decimal(1 - self.tp_pnl_percent / 1000), self.round_number)
